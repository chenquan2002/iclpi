...
class Pi0Config(_model.BaseModelConfig):
        ...
        with at.disable_typechecking():
            observation_spec = _model.Observation(
                ...
                support_caption_tokens=(
                    jax.ShapeDtypeStruct([batch_size, self.support_caption_max_len], jnp.int32)
                    if self.use_support_context and self.use_support_caption
                    else None
                ),
                support_caption_mask=(
                    jax.ShapeDtypeStruct([batch_size, self.support_caption_max_len], jnp.bool_)
                    if self.use_support_context and self.use_support_caption
                    else None
                ),
            )
    ...
    def get_vlm_freeze_filter(self) -> nnx.filterlib.Filter:
        ...
        # 冻结所有 llm 参数，但排除 action expert 分支
        return nnx.All(
            gemma_params_filter,
            nnx.Not(action_expert_params_filter),
        )
